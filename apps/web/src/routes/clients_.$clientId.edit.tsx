...
export const Route = createFileRoute("/clients_/$clientId/edit")({
...
function EditClientPage() {
	...
	useEffect(() => {
		if (client) {
			...
			const normalizedClient = {
				...
				email: client.email,
				phone: client.phone,
				taxId: (client as any).taxId || (client as any).tax_id,
				defaultHourlyRateCents: (client as any).defaultHourlyRateCents ?? (client as any).default_hourly_rate_cents,
				currency: client.currency as "USD" | "EUR" | "GBP",
				notes: client.notes,
				billingAddress: (client as any).billingAddress || (client as any).billing_address,
			};

			setFormData({
				name: normalizedClient.name,
				contactName: normalizedClient.contactName || "",
				email: normalizedClient.email || "",
				phone: normalizedClient.phone || "",
				taxId: normalizedClient.taxId || "",
				defaultHourlyRateCents: normalizedClient.defaultHourlyRateCents || undefined,
				currency: normalizedClient.currency,
				notes: normalizedClient.notes || "",
				billingAddress: normalizedClient.billingAddress || {
					line1: "",
					line2: "",
					city: "",
					region: "",
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		const submitData: CreateClient & { id: string } = {
			id: clientId,
